...
import repository.iplay.goods.GoodsRepository;
...
public class GoodsController extends BasicController {
    ...
    @Inject
    public GoodsController(Config config, AsyncCacheApi cache, WSClient ws, HttpExecutionContext httpExecutionContext, MessagesApi messagesApi, GoodsRepository repository) {
        super(config, cache, ws, httpExecutionContext, messagesApi);
        this.repository = repository;
    }

    public CompletionStage<Result> save(Http.Request request) {
        Goods modelRequest = (Goods) RequestUtil.getRequestJson(request, "model", Goods.class);
    ...
    public CompletionStage<Result> batchSave(Http.Request request) {
        List<Goods> modelRequest = RequestUtil.getRequestJsonArray(request, "models", Goods.class);
