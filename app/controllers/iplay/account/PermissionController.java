...
import repository.iplay.account.PermissionRepository;
...
public class PermissionController extends BasicController {
    ...
    @Inject
    public PermissionController(Config config, AsyncCacheApi cache, WSClient ws, HttpExecutionContext httpExecutionContext, MessagesApi messagesApi, PermissionRepository repository) {
        super(config, cache, ws, httpExecutionContext, messagesApi);
        this.repository = repository;
    }

    public CompletionStage<Result> save(Http.Request request) {
        Permission modelRequest = (Permission) RequestUtil.getRequestJson(request, "model", Permission.class);
    ...
    public CompletionStage<Result> batchSave(Http.Request request) {
        List<Permission> modelRequest = RequestUtil.getRequestJsonArray(request, "models", Permission.class);
