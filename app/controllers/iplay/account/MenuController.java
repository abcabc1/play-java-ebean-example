...
import repository.iplay.account.MenuRepository;
...
public class MenuController extends BasicController {
    ...
    @Inject
    public MenuController(Config config, AsyncCacheApi cache, WSClient ws, HttpExecutionContext httpExecutionContext, MessagesApi messagesApi, MenuRepository repository) {
        super(config, cache, ws, httpExecutionContext, messagesApi);
        this.repository = repository;
    }

    public CompletionStage<Result> save(Http.Request request) {
        Menu modelRequest = (Menu) RequestUtil.getRequestJson(request, "model", Menu.class);
    ...
    public CompletionStage<Result> saveBatch(Http.Request request) {
        List<Menu> modelRequest = RequestUtil.getRequestJsonArray(request, "models", Menu.class);
