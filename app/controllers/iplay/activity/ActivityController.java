package controllers.iplay.activity;
...
import repository.iplay.activity.ActivityRepository;
...
public class ActivityController extends BasicController {
    ...
    @Inject
    public ActivityController(Config config, AsyncCacheApi cache, WSClient ws, HttpExecutionContext httpExecutionContext, MessagesApi messagesApi, ActivityRepository repository) {
        super(config, cache, ws, httpExecutionContext, messagesApi);
        this.repository = repository;
    }

    public CompletionStage<Result> save(Http.Request request) {
        Activity modelRequest = (Activity) RequestUtil.getRequestJson(request, "model", Activity.class);
    ...
    public CompletionStage<Result> batchSave(Http.Request request) {
        List<Activity> modelRequest = RequestUtil.getRequestJsonArray(request, "models", Activity.class);
