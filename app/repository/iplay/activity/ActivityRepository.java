package repository.iplay.activity;

import io.ebean.Expr;
import io.ebean.ExpressionList;
import models.base.BaseModel;
import models.iplay.order.activity.Activity;
...
public class ActivityRepository extends BasicRepository<Activity> {

    @Inject
    public ActivityRepository(EbeanConfig ebeanConfig, DatabaseExecutionContext executionContext) {
        super(ebeanConfig, executionContext);
    }

    @Override
    public ExpressionList<? extends BaseModel> getExpr(Activity model) {
...
/*
    @Override
    public CompletionStage<Optional<Menu>> save(Activity model) {
        return supplyAsync(() -> {
            Optional<Activity> rtn = Optional.empty();
            if (model.id == null) {
                ...
                Activity modelTemp = (Activity) getModel(model);
                if (modelTemp != null) {
    ...
    public void shiftParent(Activity model) {
        ...
        if (model.parent == null || model.parent.id == null) {
            ...
            Activity parent = (Activity) getModel(model.parent);
            if (parent != null) {
    ...
    public void resetChild(Activity modelFrom, Activity modelTo, String modelClass) {
